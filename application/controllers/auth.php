...
class Auth extends Controller {
	...
	function register()
	{
		...
		{
		  $this->form_validation->set_rules('username', '用户名', 'max_length[15]|alpha_dash|required');
		  $this->form_validation->set_rules('email', '邮箱', 'valid_email|required');
		  $this->form_validation->set_rules('realname','姓名', 'required');
		  $this->form_validation->set_rules('password', '密码', 'required');
		  $this->form_validation->set_rules('code', '邀请码', 'trim|required|callback_code_verify');
		  ...
		  if($this->form_validation->run() == TRUE)
		  {
				...
				$email = $this->input->post('email');
				$password = $this->input->post('password');
				if($this->ion_auth->register($username, $password, $email, array('name'=>$this->input->post('realname'))))
				// zhen ta me de chou
				{
						$this->load->model('register_code_model');
						$this->register_code_model->use_code(trim($this->input->post('code')), $username);
						$new_user = $this->ion_auth->get_user_by_username($username);
						//generate new codes for this user
						$this->register_code_model->create_code('20', $new_user->id);
						
						$upper_id = $this->register_model->get_upper_id(trim($this->input->post('code')));
						$this->uni->log_user_invites($upper_id);
						
						$this->session->set_flashdata('user-info', '恭喜!注册成功!');
						// log in
	...
	function code_verify($code = '')
    {
        ...
        $test = $this->db->where('value',$code)->where('utime', null)->get('register_code');
        if( $test->num_rows() == 1)
        {
            return TRUE;
        }
        ...
        {
            $this->form_validation->set_message('code_verify', '该邀请码不正确');
            return FALSE;
        }
    }
